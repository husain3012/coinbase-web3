...
beforeEach(async () => {
  ...
  factory = await new web3.eth.Contract(compiledFactory.abi).deploy({ data: compiledFactory.evm.bytecode.object }).send({ from: accounts[0], gas: "3000000" });
...
describe("Campaigns", () => {
  ...
  it("requires approval before finalizing a request", async () => {
    ...
    await campaign.methods.contribute().send({ from: accounts[4], value: web3.utils.toWei("10", "ether") });
    await campaign.methods.createRequest("A", web3.utils.toWei("5", "ether"), accounts[1]).send({ from: accounts[0], gas: "1000000" });
  ...
  it("End To End", async () => {
    ...
    await campaign.methods.contribute().send({ from: accounts[4], value: web3.utils.toWei("5", "ether") });
    await campaign.methods.contribute().send({ from: accounts[5], value: web3.utils.toWei("5", "ether") });
    await campaign.methods.contribute().send({ from: accounts[6], value: web3.utils.toWei("5", "ether") });
